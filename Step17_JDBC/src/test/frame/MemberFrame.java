...
public class MemberFrame extends JFrame implements ActionListener, PropertyChangeListener{
   ...
   //테이블에 데이터 출력하는 메소드
   	public void displayMember() {
     	...
     	//int i=0 .. 등 반복문 써주는 대신 아래와 같이 써 주어도 된다.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("save")) {
			...
			if(rows.length == 0) {
				...
				//프레임이 끝나나길 원한다면. 
				//void 메소드이기 때문에 return 해줄 값이 없으므로 return 만 해주면됨.
				return;
			}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		 ...
		 * table 이 수정중이 아닐 때
		 * 현제 포커스가 있는 곳의 정보를 모두 읽어와서 DB 에 수정반영하기
		 */
		if(evt.getPropertyName().equals("tableCellEditor") && !table.isEditing()) {
			//현제 포커스가 있는 row 의 정보를 DB 에 수정 반영 한다.
